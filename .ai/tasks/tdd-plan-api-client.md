...
* `[ ]` **Setup**: Create the project structure and initialize your testing framework (e.g., Jest).

* `[ ]` **Module: `utils/parser.js`**
    * `[ ]` Create the test file: `utils/parser.test.js`.
    * `[ ]` **Test 1**: Write a test for `parseShorthandUrl` to correctly split `"post:users/1"` into `{ method: 'post', url: 'users/1' }`.
    * `[ ]` **Test 2**: Write tests for `parseInterceptorShorthand` covering all actions:
    ...
    * `[ ]` **Implementation**: Write the code in `utils/parser.js` to make all tests pass.

* `[ ]` **Module: `utils/url.js`**
    * `[ ]` Create the test file: `utils/url.test.js`.
    * `[ ]` **Test 1**: Write a test for `serializeParams` to convert `{ page: 2, sort: 'asc' }` into `?page=2&sort=asc`.
    * `[ ]` **Test 2**: Write a test to ensure it correctly URL-encodes special characters, like converting `{ q: 'hello world' }` into `?q=hello%20world`.
    * `[ ]` **Test 3**: Write a test for an empty params object, ensuring it returns an empty string.
    * `[ ]` **Implementation**: Write the code in `utils/url.js` to make all tests pass.

* `[ ]` **Module: `utils/headers.js`**
    * `[ ]` Create the test file: `utils/headers.test.js`.
    * `[ ]` **Test 1**: Write a test for `mergeHeaders` to combine multiple header objects, ensuring that properties from later objects overwrite earlier ones.
    * `[ ]` **Implementation**: Write the code in `utils/headers.js` to make all tests pass.
