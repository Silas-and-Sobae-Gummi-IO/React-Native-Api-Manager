...
buildRequestConfig.mockImplementation((cfg) => ({ url: `https://api.test.com${cfg.url}`, method: cfg.method, signal: cfg.signal }))
  ...
  it('applies headers filter and shorthand filter', async () => {
    ...
    expect(buildRequestConfig).toHaveBeenCalledWith(
      expect.objectContaining({ headers: expect.objectContaining({ authorization: 'Bearer X' }), url: '/v1/users' })
    )
  })

  it('maps timeout AbortError to ApiError and calls final', async () => {
    jest.useFakeTimers()
    const api = new ApiClient({ baseURL: 'https://api.test.com', timeout: 1000 })
...
buildRequestConfig.mockImplementation((cfg) => ({ url: `https://api.test.com${cfg.url}`, method: cfg.method, signal: cfg.signal }))
    ...
    await expect(p).rejects.toThrow('Request timed out after 1000ms')
